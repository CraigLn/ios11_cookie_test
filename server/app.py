...
def set_cookie(cookie_key):
    ...
    rsp_text = 'Cookie "' + cookie_key + '" set to ' + cookie_val + ''
    print("*"*80)
    print(rsp_text)
    print("From request with user_agent " + request.user_agent.string)
    if callback_url:
...
def delete_cookie(cookie_key):
    ...
    print("From request with user_agent " + request.user_agent.string)
    if callback_url:
        redirect_url = callback_url + "?" + cookie_key + "=" + "None"
        print("Redirecting to " + redirect_url)
        response = app.make_response(redirect(redirect_url))
    else:
        rsp_text += get_endpoints_description(cookie_key)
...
def get_cookie(cookie_key):
    ...
    callback_url = request.args.get("callbackUrl", None)
    rsp_text = 'Cookie "' + cookie_key + '" is ' + (cookie_val if cookie_val else 'not set')
    print("*"*80)
    print(rsp_text)
    print("From request with user_agent " + request.user_agent.string)
    if callback_url:
